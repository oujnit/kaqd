(function (win, doc) {
  ...
  // DeepSeek 峰谷计费：高峰为北京时间周一至周五 9:00–12:00、14:00–18:00，
  // 空闲时段价格为高峰一半（https://api-docs.deepseek.com/zh-cn/quick_start/pricing/）。
  // 若官方调整时段，需同步修改这里的窗口常量。
  function deepSeekPeakInfo(date) {
    var bj = new Date(date.getTime() + (date.getTimezoneOffset() + 480) * 60000);
    var day = bj.getDay();
    var hour = bj.getHours();
    var weekday = day >= 1 && day <= 5;
    if (weekday && ((hour >= 9 && hour < 12) || (hour >= 14 && hour < 18))) {
      return '高峰时段 · ' + (hour < 12 ? '12:00' : '18:00') + ' 后转半价';
    }
    var next;
    if (weekday && hour < 9) {
      next = '今天 9:00';
    } else if (weekday && hour >= 12 && hour < 14) {
      next = '今天 14:00';
    } else {
      var ahead = 1;
      while (!((day + ahead) % 7 >= 1 && (day + ahead) % 7 <= 5)) ahead += 1;
      next = (ahead === 1 ? '明天' : weekdays[(day + ahead) % 7]) + ' 9:00';
    }
    return '空闲时段(半价) · ' + next + ' 转高峰';
  }

  function updateDeepSeekPeriod() {
    ui.text('deepSeekPeriod', deepSeekPeakInfo(new Date()));
  }
